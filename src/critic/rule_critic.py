...
class RuleCritic:
    ...
    def check(self, post: str) -> RuleResult:
        ...
        # 2. Banned words check (case-insensitive, Unicode-aware word boundaries)
        post_lower = post.lower()
        for word in self.banned_words:
            word_lower = word.lower()
            # Unicode-aware word boundaries: ensures the banned word is not part of a larger word.
            # (?<!\w) matches if not preceded by a word character (including Unicode characters).
            # (?!\w) matches if not followed by a word character.
