...
def call_gemini_api(prompt: str, rotator: GeminiKeyRotator, model_name: str = "gemini-3.5-flash", retry_count: int = 0) -> tuple[str, dict]:
    ...
    try:
        ...
        logger.warning(f"Gemini Key index {rotator.current_idx} failed: {str(e)}. Rotating...")
        rotator.rotate()
...
"""
        ...
        # Use gemini-3.1-flash-lite as the default research model
...
"""
        ...
        logger.warning("AI Copywriter Agent: Fallback sang Gemini (gemini-3.5-flash)...")
        return call_gemini_api(prompt, self.gemini_rotator, "gemini-3.5-flash")
...
"""
        ...
        # 2. Fallback to Gemini
        logger.warning("AI LLM Critic Agent: Fallback sang Gemini (gemini-3.5-flash)...")
        content, usage = call_gemini_api(prompt, self.gemini_rotator, "gemini-3.5-flash")
        try:
            score, criticism = clean_and_parse_json(content)
            return score, criticism, usage
        except Exception as e:
